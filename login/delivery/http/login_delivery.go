...
func NewLoginHandler(e *echo.Echo, lu domain.LoginUsecase, config *viper.Viper) {
	...
	login.POST("/admin", handler.CreateJwtAdmin)
...
func (login *LoginHandler) CreateJwtUser(c echo.Context) error {
	...
	res, err := login.LoginUsecase.GetUser(ctx, &loginRequest)
	if err != nil {
		return c.String(http.StatusUnauthorized, "Your username or password were wrong")
	}

	lifetime, err := strconv.ParseInt(login.Config.GetString("LIFETIME"), 10, 64)
	if err != nil {
		lifetime = 60
	}

	secret := login.Config.GetString("SECRET_JWT")
